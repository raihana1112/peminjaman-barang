      ...
      $tgl_pinjam = date('Y-m-d');
			...
			if(in_array($ekstensi, $ekstensi_diperbolehkan) === true){
				if($ukuran < 5044070){
					...
					$query = mysqli_query($koneksi,"INSERT INTO peminjaman(id_barang, nama_peminjam, nim, foto_identitas, mata_kuliah, ruang_kuliah, dosen_pengampu, jam_mulai_mk, jam_selesai_mk, tgl_pinjam, status_ketersediaan)
               VALUES('$id_barang', '$nama_peminjam', '$nim', '$nama', '$mata_kuliah', '$ruang_kuliah', '$dosen_pengampu', '$jam_mulai', '$jam_selesai', '$tgl_pinjam', '$status_ketersediaan')");
					if($query){
            echo
            "<script>
